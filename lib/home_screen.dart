            ...
            : _messages.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    // Fix 2: show name only when first message in a consecutive group
                    final showName = !isMe && (i == 0 || _messages[i - 1].senderUid != msg.senderUid);
                    return Column(children: [
                      ...
                      _MessageBubble(
                        ...
                        myUid: user.uid, memberCount: _memberCount,
...
class _MessageBubble extends StatefulWidget {
  ...
  final bool showName;       // Fix 2: only show for first in consecutive group
  final String myUid;
  ...
  const _MessageBubble({
    ...
    required this.showAvatar, required this.showName,
    required this.myUid, required this.memberCount,
...
class _MessageBubbleState extends State<_MessageBubble> {
  ...
  void _showSeenBy(BuildContext context, ChatMessage msg) {
    final seenUids = msg.seenBy.where((u) => u != widget.myUid).toList();
    if (seenUids.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Not seen by anyone yet', style: GoogleFonts.inter(fontSize: 13))));
      return;
    }
    FirebaseFirestore.instance
        .collection('users')
        .where(FieldPath.documentId, whereIn: seenUids)
        .get()
        .then((snap) {
      if (!context.mounted) return;
      final nameMap = {for (final d in snap.docs) d.id: (d.data()['username'] as String?) ?? d.id};
      showModalBottomSheet(
        context: context,
        ...
        builder: (_) => Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('Read by', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.white)),
            const SizedBox(height: 12),
            ...seenUids.map((uid) {
              ...
              return Padding(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Text(name, style: GoogleFonts.inter(fontSize: 14, color: Colors.white))),
  ...
  void _showWhoReacted(BuildContext context, Map<String, String> reactions) {
    // Sort entries: by emoji so same emoji reactions group visually
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 12),
          ...entries.map((entry) {
            // entry.key = uid, entry.value = emoji
            // Try to find the real name from the message reactions map (uid->emoji)
            // We only have uid here; show first char of uid as avatar fallback
            final uid = entry.key;
            final emoji = entry.value;
            // Check if this uid is in the parent messages list to get a name
            // (best effort — use uid as display until we wire up a name cache)
            final displayName = uid.length > 8 ? uid.substring(0, 8) : uid;
            return Padding(
              padding: const EdgeInsets.only(bottom: 10),
              child: Row(children: [
                Container(
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isMe && !msg.isDeleted) {
      ...
      ticks = GestureDetector(
        onTap: () => _showSeenBy(context, msg),
        child: Icon(Icons.done_all_rounded, size: 14, color: allSeen ? _kTickSeen : _kTickSent),
      );
    }
              ...
              Flexible(
                child: Column(
                  ...
                  children: [
                        ...
                        child: Padding(
                          ...
                          child: IntrinsicWidth(
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                              // Sender name (others only, only first in a consecutive group)
                              if (!widget.isMe && !msg.isDeleted && widget.showName)
                                GestureDetector(
                                  onTap: () => _openProfile(context),
                                  child: Padding(
                    ...
                    // ── Reactions pill (WA-style: overlaps bubble bottom) ──
                    if (reactionCounts.isNotEmpty)
                      Transform.translate(
                        offset: const Offset(0, -8), // overlap bubble by 8px
                        child: Align(
