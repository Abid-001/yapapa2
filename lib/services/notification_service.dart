...
class NotificationService {
  ...
  static Future<void> initialize() async {
    ...
    // Create channels with correct importance
    ...
    const pokeChannel = AndroidNotificationChannel(
      _kPokeChannel, 'Pokes',
      description: 'Poke notifications',
      importance: Importance.high,
      playSound: true,
    );
    final plugin = _local
    ...
    // Handle FCM while app is in foreground
    FirebaseMessaging.onMessage.listen((msg) => showFromRemote(msg));

    // Handle FCM tap when app was in background
    FirebaseMessaging.onMessageOpenedApp.listen((msg) => showFromRemote(msg));
  }
  ...
  static Future<void> showFromRemote(RemoteMessage message) async {
    ...
    await _showLocal(title: title, body: body, channelId: type == 'preset' ? _kPresetChannel : type == 'poke' ? _kPokeChannel : _kChatChannel);
  }

  static Future<void> _showLocal({
    required String title,
    required String body,
    String channelId = _kChatChannel,
    int? id,
  }) async {
  ...
  static Future<void> showPresetMessage(String senderName, String message) => _showLocal(
    title: '🔔 $senderName',
    body: message,
    channelId: _kPresetChannel,
  );

  static Future<void> showChatMessage(String senderName, String message) => _showLocal(
    title: '💬 $senderName',
    body: message,
    channelId: _kChatChannel,
  );
