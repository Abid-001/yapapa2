...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _showOptions(BuildContext context, ChatMessage msg, bool isMe) {
    ...
    // Fix 6: save scroll position so long-press doesn't cause list to jump
    ...
    showModalBottomSheet(
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (_scrollCtrl.hasClients && (_scrollCtrl.offset - savedOffset).abs() > 20) {
            ...
            : _messages.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    // Fix 2: only show name for first msg in consecutive group
                    ...
                    return Column(children: [
                      ...
                      _MessageBubble(
                        ...
                        showAvatar: showAvatar, showName: showName, myUid: user.uid, memberCount: _memberCount,
...
class _MessageBubble extends StatefulWidget {
  ...
  const _MessageBubble({
    ...
    required this.showAvatar, required this.showName, required this.myUid, required this.memberCount,
...
class _MessageBubbleState extends State<_MessageBubble> {
  ...
  void _showSeenBy(BuildContext context, ChatMessage msg) {
    ...
    if (seenUids.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Not read by anyone yet', style: GoogleFonts.inter(fontSize:13))));
      return;
    }
    FirebaseFirestore.instance.collection('users')
      .where(FieldPath.documentId, whereIn: seenUids).get().then((snap) {
      if (!context.mounted) return;
      final nm = {for(final d in snap.docs) d.id: (d.data()['username'] as String?) ?? d.id};
      showModalBottomSheet(
        context: context, backgroundColor: Colors.transparent,
        builder: (_) => Container(
          decoration: const BoxDecoration(color: Color(0xFF1F2C34), borderRadius: BorderRadius.vertical(top: Radius.circular(20))),
  ...
  void _showWhoReacted(BuildContext context, Map<String, String> reactions) {
    final entries = reactions.entries.toList()..sort((a,b)=>a.value.compareTo(b.value));
    final uids = entries.map((e)=>e.key).toList();
    if(uids.isEmpty) return;
    FirebaseFirestore.instance.collection('users')
      .where(FieldPath.documentId, whereIn: uids).get().then((snap) {
      if(!context.mounted) return;
      final nm = {for(final d in snap.docs) d.id: (d.data()['username'] as String?) ?? d.id};
      showModalBottomSheet(
        context: context, backgroundColor: Colors.transparent,
            ...
            ...entries.map((e) {
              ...
              return Padding(
                ...
                child: Row(children: [
                  ...
                  const SizedBox(width:12),
                  Expanded(child: Text(name, style: GoogleFonts.inter(fontSize:14,color:Colors.white))),
                  Text(e.value, style: const TextStyle(fontSize:22)),
                ]),
              );
            }),
          ]),
              ...
              Flexible(
                child: Column(
                  ...
                  children: [
                        ...
                        child: Padding(
                          ...
                          child: IntrinsicWidth(
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                              // Sender name: only first msg in consecutive group
                    ...
                    // ── Reactions pill: overlaps bubble bottom (WA-style) ──
                          ...
                          child: GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Row(mainAxisSize: MainAxisSize.min, children: [
                                ...reactionCounts.keys.take(3).map((e) => Text(e, style: const TextStyle(fontSize: 14))),
                              ]),
                            ),
                          ),
                        ),
